# คู่มือ SQL และฐานข้อมูล ฉบับขยาย 126 หน้า
...
<!-- ภาคขยายต่อท้าย ตรวจแหล่งข้อมูล 2026-09-08 -->

## LATERAL และ GROUPING SETS ROLLUP CUBE

ระบบ/ขอบเขต: PostgreSQL 18

LATERAL ทำให้คิวรีย่อยใน FROM อ้างคอลัมน์ของรายการด้านซ้ายได้ เหมาะกับการคำนวณที่ขึ้นกับแต่ละแถว ส่วน GROUPING SETS ระบุชุดคอลัมน์ที่จะสรุปหลายระดับในคำสั่งเดียว ROLLUP ให้ลำดับผลรวมย่อยตามลำดับคอลัมน์ และ CUBE ให้ชุดการจัดกลุ่มทุกแบบ

LEFT JOIN LATERAL ... ON TRUE ยังเก็บแถวด้านซ้ายแม้คิวรีย่อยไม่คืนผลลัพธ์ ต่างจาก CROSS JOIN LATERAL ที่แถวนั้นอาจหายไป ควรแยก NULL ของข้อมูลจริงออกจาก NULL ที่ใช้แสดงระดับผลรวมด้วย GROUPING เมื่อรายงานต้องระบุระดับอย่างชัดเจน

ผลสาธิตและข้อจำกัด: ได้ (2,4) และ (5,10) เป็นตัวอย่างอ้างค่าด้านซ้าย ไม่จำเป็นต้องใช้ LATERAL สำหรับการคูณธรรมดา

### ตัวอย่าง

```sql
SELECT v.n, x.doubled
FROM (VALUES (2), (5)) AS v(n)
CROSS JOIN LATERAL (SELECT v.n * 2 AS doubled) AS x
ORDER BY v.n;
```
...
คู่มือฉบับรวม หน้า 102 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## CASE COALESCE NULLIF จัดการเงื่อนไขและค่าว่าง

ระบบ/ขอบเขต: PostgreSQL 18

CASE เลือกผลลัพธ์ตามเงื่อนไข COALESCE คืนอาร์กิวเมนต์ตัวแรกที่ไม่เป็น NULL และ NULLIF คืน NULL เมื่อสองอาร์กิวเมนต์เท่ากัน การเลือกใช้ช่วยแยกค่าที่ไม่ทราบออกจากศูนย์หรือข้อความว่าง ซึ่งมีความหมายต่างกัน

ผลลัพธ์แต่ละแขนงต้องแปลงเข้าชนิดข้อมูลร่วมกันได้ การใช้ NULLIF กับตัวหารช่วยให้กรณีศูนย์กลายเป็น NULL แต่ต้องกำหนดความหมายทางธุรกิจของ NULL ต่อด้วย ไม่ควรอ้างว่า CASE ป้องกันการประเมินนิพจน์ที่ผิดพลาดได้ทุกกรณี เพราะบางนิพจน์อาจถูกประเมินในขั้นวางแผน

ผลสาธิตและข้อจำกัด: label เป็น unknown, ratio เป็น NULL และ flag เป็น yes

### ตัวอย่าง

```sql
SELECT COALESCE(NULL::text, 'unknown') AS label,
       12 / NULLIF(0, 0) AS ratio,
       CASE WHEN 3 > 1 THEN 'yes' ELSE 'no' END AS flag;
```

### แหล่งอ้างอิง

https://www.postgresql.org/docs/18/functions-conditional.html
คู่มือฉบับรวม หน้า 103 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## Pagination LIMIT OFFSET และลำดับผลลัพธ์

ระบบ/ขอบเขต: PostgreSQL 18

LIMIT จำกัดจำนวนแถว ส่วน OFFSET ข้ามแถวก่อนคืนผลลัพธ์ ควรใช้ ORDER BY ที่กำหนดลำดับแน่นอน เช่น คีย์ไม่ซ้ำ เมื่อแบ่งข้อมูลเป็นหน้า มิฉะนั้นผลลัพธ์แต่ละหน้าไม่จำเป็นต้องเป็นชุดเดิม

แถวที่ถูกข้ามด้วย OFFSET ยังต้องถูกคำนวณ จึงอาจช้าเมื่อข้ามจำนวนมาก ตัวอย่างนี้ใช้ข้อมูลคงที่ หากข้อมูลเปลี่ยนระหว่างคำขอ การมี ORDER BY เพียงอย่างเดียวไม่ได้ทำให้ทุกหน้าอยู่ใน snapshot เดียวกัน

ผลสาธิตและข้อจำกัด: ได้ 2 และ 3 ตามลำดับ ไม่มีการรับประกันว่าคิวรีไม่ใส่ ORDER BY จะเรียงตามการบันทึก
...
คู่มือฉบับรวม หน้า 104 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## ERD Cardinality ความสัมพันธ์ Many-to-many และ Constraints

ระบบ/ขอบเขต: PostgreSQL 18

Primary key ระบุแถวไม่ซ้ำและไม่เป็น NULL ส่วน foreign key รักษาความสัมพันธ์กับแถวอ้างอิง ความสัมพันธ์หลายต่อหลายเก็บผ่านตารางกลางที่มี foreign key ไปยังทั้งสองฝั่ง และใช้คีย์ร่วมป้องกันคู่ซ้ำได้

CHECK ใช้ตรวจเงื่อนไขของแถว แต่การได้ NULL จากเงื่อนไข CHECK ไม่เท่ากับละเมิดเงื่อนไข ถ้าห้ามค่าว่างต้องใส่ NOT NULL ด้วย ควรเลือก ON DELETE ให้ตรงอายุข้อมูล ไม่เปิด CASCADE เพียงเพื่อทำให้ข้อผิดพลาดหายไป

ผลสาธิตและข้อจำกัด: ตัวอย่างแสดงการกันคู่ซ้ำเท่านั้น ยังไม่มีตารางนักเรียนและวิชาหรือ foreign key ไปยังตารางจริง

### ตัวอย่าง

```sql
CREATE TEMP TABLE demo_enrollment (
...
คู่มือฉบับรวม หน้า 105 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## NUMERIC DECIMAL Precision Scale และ Floating Point

ระบบ/ขอบเขต: PostgreSQL 18

integer เหมาะกับจำนวนเต็ม ส่วน numeric หรือ decimal เก็บค่าฐานสิบแบบแม่นตรงตามข้อกำหนด precision และ scale ขณะที่ real และ double precision เป็นชนิดประมาณค่า จึงไม่ควรคาดหวังความเท่ากันแบบฐานสิบทุกกรณี

precision คือจำนวนหลักทั้งหมด ส่วน scale เกี่ยวกับหลักทศนิยม การบันทึกค่าใน numeric ที่กำหนด scale อาจเกิดการปัดเศษ และค่าที่เกินจำนวนหลักที่รองรับอาจถูกปฏิเสธ ต้องเลือกชนิดตามช่วงค่าและกฎการปัดเศษของงาน

ผลสาธิตและข้อจำกัด: ได้ 12.35 เป็นผลของการปรับ scale เป็นสองตำแหน่งใน PostgreSQL

### ตัวอย่าง

```sql
SELECT CAST(12.345 AS numeric(5,2)) AS amount;
```

### แหล่งอ้างอิง

https://www.postgresql.org/docs/18/datatype-numeric.html
คู่มือฉบับรวม หน้า 106 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## Composite Multicolumn Index ลำดับคอลัมน์

ระบบ/ขอบเขต: PostgreSQL 18

ดัชนีหลายคอลัมน์ช่วยคิวรีที่กรองหลายเงื่อนไข แต่ลำดับคอลัมน์มีผลต่อประสิทธิภาพ โดยทั่วไป B-tree ได้ประโยชน์มากจากข้อจำกัดบนคอลัมน์นำหน้า ก่อนพิจารณาเงื่อนไขช่วงของคอลัมน์ถัดมา

PostgreSQL 18 อาจใช้ skip scan บางกรณี จึงไม่ควรสรุปว่าขาดเงื่อนไขคอลัมน์แรกแล้วใช้ดัชนีไม่ได้เสมอ การเลือกดัชนีต้องดูชนิดดัชนี การกระจายค่า และแผนคิวรีจริง การเพิ่มดัชนีมีต้นทุนด้านพื้นที่และการเขียน

ผลสาธิตและข้อจำกัด: สร้างดัชนีในตารางชั่วคราว ตัวอย่างไม่ได้รับรองว่า planner จะเลือกใช้ดัชนีเมื่อมีข้อมูลน้อย

### ตัวอย่าง

```sql
CREATE TEMP TABLE demo_events (tenant_id integer, event_id integer);
CREATE INDEX demo_events_lookup ON demo_events (tenant_id, event_id);
```

### แหล่งอ้างอิง

https://www.postgresql.org/docs/18/indexes-multicolumn.html
คู่มือฉบับรวม หน้า 107 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## Partial Index ดัชนีเฉพาะบางแถว

ระบบ/ขอบเขต: PostgreSQL 18

Partial index เก็บดัชนีเฉพาะแถวที่ผ่าน predicate เหมาะเมื่อคิวรีสนใจข้อมูลบางส่วน เช่น งานที่ยังไม่ปิด ช่วยลดขนาดเมื่อเทียบกับการทำดัชนีทุกแถวในกรณีที่เหมาะสม

Planner ต้องพิสูจน์ได้ว่าเงื่อนไขคิวรีสอดคล้องกับ predicate ของดัชนี เงื่อนไขที่เขียนต่างรูปหรือค่าพารามิเตอร์ซึ่งยังไม่ทราบตอนวางแผนอาจทำให้ใช้ไม่ได้ ไม่ควรสร้าง partial index จำนวนมากเพื่อเลียนแบบ partition โดยไม่วิเคราะห์งาน

ผลสาธิตและข้อจำกัด: ดัชนีครอบคลุมเฉพาะแถว closed=false การค้นงานปิดแล้วไม่ได้รับประโยชน์จากดัชนีนี้โดยตรง

### ตัวอย่าง

```sql
CREATE TEMP TABLE demo_jobs (id integer, closed boolean NOT NULL);
CREATE INDEX demo_jobs_open ON demo_jobs (id) WHERE closed = false;
```

### แหล่งอ้างอิง

https://www.postgresql.org/docs/18/indexes-partial.html
คู่มือฉบับรวม หน้า 108 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## VACUUM ANALYZE Autovacuum และ Dead Tuples

ระบบ/ขอบเขต: PostgreSQL 18

เมื่อ UPDATE หรือ DELETE รุ่นแถวเก่าอาจยังต้องอยู่ให้ธุรกรรมอื่นอ่าน VACUUM จัดการรุ่นแถวที่ไม่จำเป็นแล้วเพื่อให้พื้นที่กลับมาใช้ซ้ำ ส่วน ANALYZE เก็บสถิติช่วย planner ประเมินจำนวนแถวและเลือกแผน

VACUUM ปกติไม่รับประกันว่าขนาดไฟล์จะลดลงและคืนพื้นที่ทั้งหมดให้ระบบปฏิบัติการ ส่วน VACUUM FULL เขียนตารางใหม่และต้องใช้ lock ที่แรงกว่า Autovacuum ช่วยดูแลงานเหล่านี้ แต่ธุรกรรมยาวและภาระเขียนสูงยังควรได้รับการติดตาม

ผลสาธิตและข้อจำกัด: อ่านค่าประมาณ dead tuples และเวลา autovacuum ไม่ได้สั่ง vacuum หรือแก้ข้อมูลจริง
...
คู่มือฉบับรวม หน้า 109 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## Monitoring pg_stat_activity Wait Events

ระบบ/ขอบเขต: PostgreSQL 18

pg_stat_activity แสดงสถานะของกระบวนการเชื่อมต่อ เช่น active หรือ idle in transaction ส่วน wait_event_type และ wait_event ช่วยบอกการรอ การติดตามสถานะร่วมกับระยะเวลาธุรกรรมช่วยแยกการทำงานออกจากการติดค้าง

สถิติสะสมบางชุดไม่ได้อัปเดตทันที และการอ่านในธุรกรรมอาจใช้ snapshot เดิม ผู้ใช้ทั่วไปเห็นรายละเอียดของ session อื่นได้จำกัด อย่าตัดสินว่าคิวรีผิดหรือยกเลิก session จากค่าเดียวโดยไม่ตรวจบริบท

ผลสาธิตและข้อจำกัด: เป็นคำสั่งอ่านเพื่อสังเกตระบบ ไม่ยกเลิก session และไม่เปลี่ยนการตั้งค่า
...
คู่มือฉบับรวม หน้า 110 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## WAL Write-Ahead Log และ Crash Recovery

ระบบ/ขอบเขต: PostgreSQL 18

หลักการ WAL คือบันทึกข้อมูลสำหรับทำซ้ำการเปลี่ยนแปลงลงสื่อถาวรก่อนเขียนหน้าข้อมูลที่เกี่ยวข้อง เมื่อระบบล้มเหลวสามารถใช้ log ทำ REDO กับการเปลี่ยนแปลงที่ยังไม่ปรากฏครบในไฟล์ข้อมูล

WAL ช่วยลดความจำเป็นที่จะต้องเขียนทุกหน้าข้อมูลลงดิสก์ตอน commit แต่ความทนทานยังขึ้นกับการตั้งค่าและความถูกต้องของระบบจัดเก็บ WAL ไม่ใช่การสำรองข้อมูลสมบูรณ์ด้วยตัวมันเอง ต้องออกแบบ backup และการเก็บ log ร่วมกัน

ผลสาธิตและข้อจำกัด: แสดงระดับ WAL ปัจจุบัน ไม่ได้เปลี่ยนการตั้งค่า และค่าที่ได้ขึ้นกับเซิร์ฟเวอร์
...
คู่มือฉบับรวม หน้า 111 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## Logical Replication Publication Subscription

ระบบ/ขอบเขต: PostgreSQL 18

Logical replication ส่งการเปลี่ยนแปลงตามตัวตนของข้อมูลผ่าน publication และ subscription ต่างจาก physical replication ที่อิงการเปลี่ยนแปลงระดับกายภาพของคลัสเตอร์ ใช้เลือกตารางที่ต้องส่งต่อได้

โดยทั่วไปมีการคัดลอกข้อมูลเริ่มต้นแล้วตามด้วยการเปลี่ยนแปลงต่อเนื่อง การใช้งานจริงต้องตรวจ replica identity ชนิดข้อมูล สิทธิ์ และข้อจำกัดของรุ่น ไม่ควรถือว่าการตั้ง publication เพียงอย่างเดียวเท่ากับระบบ failover หรือ backup ที่สมบูรณ์

ผลสาธิตและข้อจำกัด: แสดง publication ที่มีอยู่ หากยังไม่ได้ตั้งค่าอาจได้ศูนย์แถว ตัวอย่างไม่สร้างระบบ replication
...
คู่มือฉบับรวม หน้า 112 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## Schema Namespace search_path และชื่อเต็ม

ระบบ/ขอบเขต: PostgreSQL 18

Schema เป็น namespace ภายในฐานข้อมูล ทำให้มีตารางชื่อเดียวกันในคนละ schema ได้ ชื่อเต็มแบบ schema.table ช่วยระบุวัตถุชัดเจน ส่วน search_path กำหนดลำดับค้นหาวัตถุที่ไม่ได้ระบุ schema

การใส่ schema ที่ผู้อื่นสร้างวัตถุได้ไว้ใน search_path หมายถึงต้องเชื่อถือผู้ที่มีสิทธิ์นั้น ควรจำกัด CREATE และใช้ชื่อเต็มในส่วนที่อ่อนไหว Schema ไม่ใช่การแยกข้อมูลด้วยสิทธิ์โดยอัตโนมัติ ต้องกำหนดสิทธิ์เข้าถึงด้วย

ผลสาธิตและข้อจำกัด: แสดงเส้นทางค้นหาและ schema ปัจจุบันโดยไม่แก้ไขสิทธิ์หรือสร้างวัตถุ
...
คู่มือฉบับรวม หน้า 113 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## COPY CSV Bulk Import Export

ระบบ/ขอบเขต: PostgreSQL 18

COPY ใช้รับหรือส่งข้อมูลจำนวนมากระหว่างตารางหรือผลคิวรีกับไฟล์หรือช่องทางมาตรฐาน รองรับรูปแบบ CSV พร้อมตัวเลือก delimiter, header และค่าแทน NULL การกำหนดคอลัมน์ชัดเจนช่วยควบคุมลำดับข้อมูล

COPY ที่ระบุชื่อไฟล์เข้าถึงไฟล์จากฝั่งเซิร์ฟเวอร์ ไม่ใช่คอมพิวเตอร์ผู้ใช้โดยอัตโนมัติ ต้องตรวจสิทธิ์ การเข้ารหัส และความหมายของค่าว่างก่อนนำเข้า คำสั่งนำเข้ามีผลเขียนข้อมูลจึงควรใช้ตารางทดลองก่อน

ผลสาธิตและข้อจำกัด: ส่งหัวคอลัมน์ id,label และข้อมูลหนึ่งแถวออกช่องทาง STDOUT ของ client ที่รองรับ COPY ไม่เขียนไฟล์เซิร์ฟเวอร์

### ตัวอย่าง

```sql
COPY (SELECT 1 AS id, 'sample' AS label)
...
คู่มือฉบับรวม หน้า 114 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## Full Text Search tsvector tsquery

ระบบ/ขอบเขต: PostgreSQL 18

Full text search แปลงข้อความเป็นรูปที่ค้นหาเชิงคำได้ โดย tsvector เก็บเอกสารที่ผ่านการปรับรูปคำ และ tsquery แทนเงื่อนไขค้นหา ตัวดำเนินการ @@ ตรวจว่าเอกสารตรงกับคำค้นหรือไม่

ผลการตัดคำ การลดรูป และ stop words ขึ้นกับ text search configuration ตัวอย่างภาษาอังกฤษไม่ได้รับรองความสามารถตัดคำไทย ต้องตรวจเครื่องมือและพจนานุกรมของภาษาที่ใช้งาน ไม่ควรเทียบความหมายกับการค้น substring แบบ LIKE โดยตรง

ผลสาธิตและข้อจำกัด: ได้ true ภายใต้ configuration english เป็นตัวอย่างการลดรูปคำ ไม่ใช่การค้นความหมายด้วย AI

### ตัวอย่าง

```sql
SELECT to_tsvector('english', 'databases are useful')
       @@ plainto_tsquery('english', 'database') AS matched;
```

### แหล่งอ้างอิง

https://www.postgresql.org/docs/18/textsearch-intro.html
คู่มือฉบับรวม หน้า 115 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## CREATE FUNCTION RETURNS Volatility

ระบบ/ขอบเขต: PostgreSQL 18

Function มีชื่อ พารามิเตอร์ และชนิดผลลัพธ์ ใช้เป็นส่วนหนึ่งของนิพจน์ SQL ได้ PostgreSQL รองรับภาษาและรูปแบบการคืนค่าหลายแบบ ต้องระบุคุณสมบัติการเปลี่ยนแปลงผลลัพธ์ให้ตรงพฤติกรรมจริง

IMMUTABLE หมายถึงอินพุตเดียวกันให้ผลเดียวกันเสมอ STABLE และ VOLATILE มีข้อกำหนดต่างกัน การติดป้ายผิดอาจทำให้ planner ใช้ผลไม่ถูกต้อง SECURITY DEFINER ต้องระวังสิทธิ์เจ้าของและ search_path เป็นพิเศษ

ผลสาธิตและข้อจำกัด: สร้างฟังก์ชันใน temporary schema ของ session และคืนค่า 14 เป็นแบบฝึกหัด ไม่ใช้สิทธิ์ SECURITY DEFINER

### ตัวอย่าง

```sql
CREATE FUNCTION pg_temp.demo_double(n integer)
RETURNS integer LANGUAGE SQL IMMUTABLE
AS 'SELECT n * 2';
SELECT pg_temp.demo_double(7);
```

### แหล่งอ้างอิง

https://www.postgresql.org/docs/18/sql-createfunction.html
คู่มือฉบับรวม หน้า 116 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## CREATE PROCEDURE CALL Stored Procedure

ระบบ/ขอบเขต: PostgreSQL 18

Procedure เป็น routine ที่เรียกด้วย CALL ต่างจาก function ที่ใช้ในนิพจน์และมีชนิดผลลัพธ์ผ่าน RETURNS จึงควรเลือกชนิด routine ตามวิธีเรียกและงานที่ต้องทำ ไม่ใช้ไวยากรณ์ของ SQL Server หรือ MySQL มาปะปนโดยตรง

สิทธิ์สร้าง routine ภาษาที่อนุญาต และสิทธิ์ EXECUTE ต้องเหมาะสม Procedure บางรูปแบบมีข้อจำกัดเรื่อง transaction control โดยเฉพาะเมื่อใช้ SECURITY DEFINER หรือ SET clause จึงไม่ควรสรุปว่าทุก procedure สามารถ COMMIT ได้ทุกบริบท

ผลสาธิตและข้อจำกัด: เรียก procedure แบบไม่มีผลแก้ข้อมูล CALL นี้ไม่ได้คืนตารางผลลัพธ์เหมือน SELECT function

### ตัวอย่าง

```sql
CREATE PROCEDURE pg_temp.demo_noop()
LANGUAGE SQL AS 'SELECT 1';
CALL pg_temp.demo_noop();
```

### แหล่งอ้างอิง

https://www.postgresql.org/docs/18/sql-createprocedure.html
คู่มือฉบับรวม หน้า 117 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## MySQL InnoDB ACID Storage Engine

ระบบ/ขอบเขต: MySQL 8.4

InnoDB เป็น storage engine ที่รองรับธุรกรรม การ commit rollback และ crash recovery มี row-level locking และ consistent reads เพื่อรองรับการใช้งานพร้อมกัน รวมถึง foreign key เพื่อรักษาความสัมพันธ์

คุณสมบัติของ storage engine ไม่ควรถูกเหมารวมกับทุกตารางใน MySQL โดยไม่ตรวจ engine จริง การออกแบบ primary key มีความสำคัญกับการจัดเก็บแบบ clustered ของ InnoDB และความทนทานต้องพิจารณาการตั้งค่าระบบด้วย

ผลสาธิตและข้อจำกัด: แสดง engine ของตารางในฐานข้อมูลที่เลือก ต้องเลือกฐานข้อมูลก่อน มิฉะนั้นเงื่อนไขอาจไม่คืนข้อมูล
...
คู่มือฉบับรวม หน้า 118 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## MySQL SQL_MODE Strict Mode ONLY_FULL_GROUP_BY

ระบบ/ขอบเขต: MySQL 8.4

SQL mode กำหนดพฤติกรรมตรวจสอบและไวยากรณ์บางส่วนของ MySQL เช่น strict mode ที่มีผลต่อข้อมูลไม่ถูกต้อง และ ONLY_FULL_GROUP_BY ที่มีผลกับคิวรี aggregate และคอลัมน์ที่ไม่ได้จัดกลุ่ม

ข้อผิดพลาดที่เกิดเฉพาะบางเครื่องอาจมาจาก session mode หรือ global mode ต่างกัน ควรตรวจค่าและแก้คิวรีให้ชัดเจนก่อนปิดข้อจำกัด การปิด strict mode อาจเปลี่ยนข้อผิดพลาดบางแบบเป็นคำเตือนหรือการปรับค่า จึงต้องประเมินผลต่อความถูกต้อง

ผลสาธิตและข้อจำกัด: อ่านค่าระดับ session และ global โดยไม่เปลี่ยนการตั้งค่าเซิร์ฟเวอร์
...
คู่มือฉบับรวม หน้า 119 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## SQL Server OFFSET FETCH แบ่งหน้า T-SQL

ระบบ/ขอบเขต: SQL Server

T-SQL ใช้ OFFSET และ FETCH ร่วมกับ ORDER BY เพื่อข้ามและจำกัดแถวในการแบ่งหน้า ไม่ใช้ LIMIT แบบ PostgreSQL หรือ MySQL ควรเรียงด้วยคอลัมน์หรือชุดคอลัมน์ที่ให้ลำดับไม่กำกวม

ผลการแบ่งหน้าที่คงที่ต้องพิจารณาทั้งลำดับไม่ซ้ำและการเปลี่ยนข้อมูลระหว่างคำขอ ไวยากรณ์ ORDER BY ใน view หรือคิวรีย่อยมีข้อจำกัด และไม่ได้รับรองลำดับของคำสั่งภายนอกถ้าภายนอกไม่สั่งเรียง

ผลสาธิตและข้อจำกัด: ได้ id=20 ตัวอย่างนี้เป็น T-SQL และไม่อ่านตารางของผู้ใช้

### ตัวอย่าง

```sql
SELECT v.id FROM (VALUES (10), (20), (30)) AS v(id)
ORDER BY v.id OFFSET 1 ROWS FETCH NEXT 1 ROWS ONLY;
```

### แหล่งอ้างอิง

https://learn.microsoft.com/en-us/sql/t-sql/queries/select-order-by-clause-transact-sql?view=sql-server-ver17
คู่มือฉบับรวม หน้า 120 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## SQLite Foreign Keys PRAGMA foreign_keys

ระบบ/ขอบเขต: SQLite

การมี REFERENCES ใน schema ไม่ได้แปลว่าทุก connection กำลังบังคับ foreign key ต้องตรวจ PRAGMA foreign_keys และความสามารถของ SQLite build ที่ใช้ เปิดการตรวจสำหรับ connection ก่อนเริ่มธุรกรรมที่ต้องพึ่งข้อจำกัดนี้

การเปลี่ยน foreign_keys ระหว่างธุรกรรมที่กำลังทำงานไม่มีผลตามที่คาด Foreign key อ้าง parent key ที่มีข้อกำหนด uniqueness และ collation เหมาะสม ควรทดสอบแถวลูกที่ไม่มีแถวแม่และพฤติกรรมตอนลบ

ผลสาธิตและข้อจำกัด: เมื่อ build รองรับและรันนอกธุรกรรม การตรวจค่าควรคืน 1 เป็นการตั้งค่าของ connection ไม่ใช่ทุกโปรแกรมที่เปิดไฟล์เดียวกัน
...
คู่มือฉบับรวม หน้า 121 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## SQLite WAL Concurrency และ Checkpoint

ระบบ/ขอบเขต: SQLite

WAL mode ให้การอ่านและการเขียนเกิดพร้อมกันได้มากกว่ารูปแบบ rollback journal ในหลายสถานการณ์ แต่ยังมีผู้เขียนได้ครั้งละหนึ่งราย Checkpoint ย้ายการเปลี่ยนแปลงจาก WAL กลับไปยังฐานข้อมูลหลัก

ผู้อ่านที่เปิดธุรกรรมไว้นานอาจขัดขวางความคืบหน้าของ checkpoint และทำให้ WAL โตขึ้น WAL อาศัยการประสานงานบนเครื่องเดียวกัน ไม่เหมาะกับการใช้ไฟล์ร่วมผ่าน network filesystem ต้องตรวจข้อจำกัดและ SQLite รุ่นที่ใช้

ผลสาธิตและข้อจำกัด: อ่าน journal mode ปัจจุบันเท่านั้น ไม่สั่งเปลี่ยนไฟล์ฐานข้อมูลไปใช้ WAL
...
คู่มือฉบับรวม หน้า 122 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## MongoDB Schema Validation JSON Schema

ระบบ/ขอบเขต: MongoDB

โครงสร้างเอกสารที่ยืดหยุ่นไม่ได้หมายความว่าต้องยอมรับข้อมูลทุกชนิด MongoDB กำหนด validation rules ของ collection ได้ เช่น ชนิดข้อมูลและฟิลด์ที่ต้องมี โดยรองรับ JSON Schema บางส่วนตามเอกสารของผลิตภัณฑ์

เมื่อเอกสารใหม่หรือการอัปเดตผิดกฎ พฤติกรรมขึ้นกับ validation action และ validation level ที่ตั้งไว้ การเพิ่มกฎไม่ใช่การแก้เอกสารเก่าย้อนหลังให้ถูกทั้งหมด ต้องวางแผนตรวจและย้ายข้อมูลเดิมร่วมด้วย

ผลสาธิตและข้อจำกัด: ไม่มีตัวอย่างคำสั่ง MongoDB ในหัวข้อนี้ แชทจึงไม่สร้างคำสั่งเพิ่มเอง

### แหล่งอ้างอิง

https://www.mongodb.com/docs/manual/core/schema-validation/
คู่มือฉบับรวม หน้า 123 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## Data Warehouse Star Schema Fact Dimension Grain

ระบบ/ขอบเขต: Power BI / Dimensional modeling

Star schema แยก fact สำหรับเหตุการณ์หรือค่าที่ต้องวัดออกจาก dimension สำหรับอธิบายและกรอง เช่น เวลา สินค้า และลูกค้า ต้องกำหนด grain ว่าหนึ่งแถวของ fact แทนเหตุการณ์ระดับใดก่อนออกแบบมาตรวัด

Dimension ควรมีคีย์ที่รองรับความสัมพันธ์ฝั่งหนึ่งต่อหลาย การจัดการ slowly changing dimension ต้องเลือกว่าจะทับค่าปัจจุบันหรือเก็บประวัติหลายรุ่น การรวมยอดโดยไม่เข้าใจ grain อาจนับซ้ำ โดยเฉพาะเมื่อเชื่อมตารางที่มีหลายแถวต่อคีย์

ผลสาธิตและข้อจำกัด: ไม่มี SQL ตัวอย่างที่สมมติโครงสร้างคลังข้อมูลจริง เนื้อหานี้อธิบายแนวคิด ไม่ใช่การสร้าง warehouse สำเร็จรูป

### แหล่งอ้างอิง

https://learn.microsoft.com/en-us/power-bi/guidance/star-schema
คู่มือฉบับรวม หน้า 124 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ

## TLS SSL verify-full การเข้ารหัสการเชื่อมต่อ

ระบบ/ขอบเขต: PostgreSQL libpq

libpq กำหนด sslmode เพื่อควบคุมการเชื่อมต่อ TLS ได้ verify-ca ตรวจสายความเชื่อถือของใบรับรอง ส่วน verify-full ตรวจเพิ่มว่าชื่อโฮสต์ตรงกับใบรับรอง เหมาะเมื่อต้องยืนยันทั้งผู้ให้บริการและปลายทาง

การเข้ารหัสช่องทางอย่างเดียวไม่เท่ากับยืนยันตัวตนเซิร์ฟเวอร์ ต้องจัดเตรียม CA และชื่อโฮสต์ให้ถูกต้อง อย่าแก้ปัญหาใบรับรองด้วยการปิดการตรวจใน production และอย่าใส่รหัสผ่านหรือ API key ลงตัวอย่างที่แชร์

ผลสาธิตและข้อจำกัด: ไม่มี connection string ที่ใส่ข้อมูลลับ ตัวเลือก sslmode เป็นของ libpq; driver อื่นอาจใช้ชื่อตัวเลือกต่างกัน

### แหล่งอ้างอิง

https://www.postgresql.org/docs/18/libpq-ssl.html
คู่มือฉบับรวม หน้า 125 · ตรวจเอกสาร 2026-09-08 · เรียบเรียงภาษาไทยและตัวอย่างสาธิตโดยผู้จัดทำ
